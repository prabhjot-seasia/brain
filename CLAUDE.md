...
5. **Jira Ticket Creation** (Phase 3): `POST /api/v1/jira/tickets/propose` → `JiraTicketController` → `TicketProposalService` (LLM decomposes content into tickets) → user reviews → `POST /api/v1/jira/tickets/create` → `TicketCreationService` → `JiraClient` creates tickets via Atlassian REST API. Auth is a single service-account PAT (Atlassian Cloud API token) — `BRAIN_JIRA_BASE_URL` + `BRAIN_JIRA_EMAIL` + `BRAIN_JIRA_API_TOKEN` — sent as HTTP Basic. `JiraConfigController` exposes `GET /api/v1/jira/config` so the UI can render clickable issue links against the configured base URL.
...
- **Controllers** (feature packages): `IntakeController` (`intake/`), `DocGeneratorController` (`docs/`), `JiraTicketController` (`jira/`), `JiraConfigController` (`jira/`), `GitHubWebhookController` (`ci/`)
...
- **PostgreSQL 16 + pgvector**: projects, chunks (1024-dim HNSW index — shared between Ollama bge-m3 and AWS Bedrock Titan v2), clarification_sessions (JSONB rounds + final_plan), pull_request_records, ci_remediation_attempts, intake_records, generated_documents, ticket_proposals, token_usage_records
...
- **Jira PAT**: `BRAIN_JIRA_BASE_URL` (e.g. `https://yourorg.atlassian.net`), `BRAIN_JIRA_EMAIL`, `BRAIN_JIRA_API_TOKEN` (Atlassian Cloud API token) — service-account credentials sent as HTTP Basic for all outbound Jira calls (read issues, create issues, post comments, label transitions)
